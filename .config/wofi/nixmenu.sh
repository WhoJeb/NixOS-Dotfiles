...
selected=$(echo -e $entries|wofi --width 250 --height 500 --dmenu --cache-file /dev/null | awk '{print tolower($2)}')

spawn_float() {
  niri msg action spawn -- kitty --title "NixOS Menu" sh -c "$1; exec bash"
}
